...
As always, a more indepth literature review will need to continue. 
	...
	So I'm working on the Raspi version now. So far I'm working with Raspbian Jessie on a RasPi 3. 

	I'm using the repo versions of GNU Radio and UHD. 

	Now I'm trying to install gr-foo. To fix the first errors I got I had to run these:

	\shellcmd{sudo apt-get install gnuradio-dev}
	\shellcmd{sudo apt-get install libcppunit-dev}
	\shellcmd{sudo apt-get install liblog4cpp5-dev}

	Then to get the system to recognize the blocks I had to move the .xml files located in:

	\shellcmd{gr-foo/grc}
	To 
	\shellcmd{~/.grc\_gnuradio/}

	I simply copied them all by typing the following from the gr-foo/grc directory:
	\shellcmd{ cp * ~/.grc\_gnuradio/}

	Next I will attempt to install gr-mac. Again we are using Balint's fork not Malsbury's original:

	\href{https://github.com/balint256/gr-mac.git}{https://github.com/balint256/gr-mac.git}

	Same as before, we need to move the files in gr-mac/grc into ~/.grc\_gnuradio/

	I also had to copy all of these files to the same directory in the root folder. However, I'm not getting a hardware related error. Hopefully I'll have time to debug later. 

	\section{4-11-16}

	After doing the steps mentioned in the previous section, we still had a few issues. It ended up being that the UHD drivers needed to be updated to the latest. This removed all errors and the system began functioning on the RasPi's. However, it should be noted that I needed to run non-GUI versions. The GUI is prety computationally intensive so it worked a lot better once we killed that. 

	This was all done on the RasPi 3's, but it would be good to try this again with a raspi 2. 
