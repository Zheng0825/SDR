...
ARCAM-Net is a Software Defined Radio Network (SDRN) testbed and platform. ARCAM-Net creates a fully functioning data network capable of acting as a multi-hop mesh network. Nearly every component of ARCAM-Net, except the Software Defined Radios (SDRs) themselves, is released using an open source license. The goal of ARCAM-Net is to establish a low cost platform that can be quickly implemented by anyone interested in experimenting with SDRNs. The system provides a simple user intrface for interacting with the network. User space applications can be run on the network as if it was a traditional network.This document presents the network itself and also acts as a manual for working with ARCAM's first implementation. 
