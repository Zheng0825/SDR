...
#include <test_11n.h>
#include "../src/tx/transmission_interface_11n.h"
#include "../src/rx/data_decoding_11n.h"
#include "../src/rx/header_decoding_11n.h"
...
endl;
	...
	{	
		...
		for (; tap_index<number_of_taps; tap_index++)
		{
			...
			output_stream.set_submatrix(0,tap_index+zero_padding_length,generic_matrix*input_stream+output_stream.get_cols(tap_index+zero_padding_length,zero_padding_length+input_stream_length+tap_index-1));
		}
		...
		for (; snr_index<length(snr_values); snr_index++)
		{
			...
			awgn.set_noise(channel_power/double(snr_values(snr_index)));
				...
				else
				{
					...
					*/
					if (verbose) 
					{
						...
						cout << "------> Spatial SNR Matrix: " << endl << 10*log10(rdp.get_spatial_snr()) << endl;
					...
					// If BER mode, analyze the BER
					if (is_BER_on)
					{
						// assign BER matrix
						BER_matrix(0,snr_index)+=berc.get_errors();
						berc.clear();
						berc.count(tx_front_end.get_coded_bits(),rdp.get_coded_bits());
						BER_matrix(1,snr_index)+=berc.get_errors();
					}
				}	
			}		
		}
	...
	if (verbose)
	{
		cout << "ADMIN: CHANNEL SIMULATION STATISTICS" << endl;
		vec sing_values;
		for (unsigned int condindex=0; condindex<observed_channels.length(); condindex++)
		{
			cout << "------> Tap " << condindex+1 << ": ";
			for (unsigned int dimindex=0; dimindex<observed_channels(0).cols()*observed_channels(0).rows(); dimindex++)
			{
				cout << "[h" << (dimindex%observed_channels(condindex).rows()+1)
<< (dimindex/observed_channels(condindex).rows()+1) << "=" << observed_channels(condindex)(dimindex%observed_channels(condindex).rows(),dimindex/observed_channels(condindex).rows()) << "]  ";
			}
			cout << endl;
			if (observed_channels(0).rows()==observed_channels(0).cols())
			{
				sing_values=svd(observed_channels(condindex));
				cout << "------> Singular Values: " << sing_values << endl;
				cout << "------> Condition Number: [" << std::abs(sing_values(0))/std::abs(sing_values(length(sing_values)-1)) << "]" << endl;
				cout << "------> Demmel Condition Number: ";
				cout << "[" << (norm(observed_channels(condindex),"fro")*norm(inv(observed_channels(condindex)),2)) << "] ";
				cout << endl;
			}
		}
	}
