...
#ifndef TEST_80211N_H
#define TEST_80211N_H
...
window) {coarse_window=window;}
		...
		/*! \brief Set the gain control to be used in the header decode
		 *
		 *  \param gain_mode 0 -> channel averaging (average rx channels
		 *  together), 1 -> energy ratio combining the summed energy of each
		 *  channel depends on the energy observed in each rx channel
		 */
		inline void set_gain_control(unsigned int ga_mode)
{gain_mode=ga_mode;}
...
{equalization_mode=eq_mode;}
		...
		inline itpp::mat get_post_processing_snr() {return ppsnr;}
		inline itpp::Array<itpp::cmat> get_extension_channel() {return extchan;}
		inline double get_channel_condition() {return chan_cond;}
		inline double get_avg_snr() {return avg_snr;}
