...
RateControlRSSI::RateControlRSSI()
...
void*
RateControlRSSI::cast(const char *n)
{
  if( strcmp(n, "RateControlRSSI") == 0 )
    return (RateControlRSSI *) this;
  else
    return RateControl::cast(n);
}

int
RateControlRSSI::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
Packet *
RateControlRSSI::simple_action(Packet *p_in)
{
...
void
RateControlRSSI::set_txrate(Packet* p_in)
{
...
WritablePacket*
RateControlRSSI::get_txrate(Packet* p_in)
{
...
void RateControlRSSI::init_tables()
{
...
void RateControlRSSI::interpolate_bers(double snr, vector<double>& bers)
{
...
unsigned short 
RateControlRSSI::calculate_bers(unsigned int , vector<double>& bers)
{
  ...
  //printf("RateControlRSSI:: cutoff = %f, per = %f, rate = %d\n", cutoff, per, rate);
  //hydra_chatter(8, _verbose, "%{element}:calculate_pers: per = %f rate = %d\n", this, per, rate); 
  return rate; 
}

unsigned short 
RateControlRSSI::calculate_pers(unsigned int len, vector<double>& bers)
{
  unsigned short rate = 6;
  double per = 1.0;
  double bsr = 1.0;
...
unsigned short 
RateControlRSSI::calculate_datarate_ber(double snr, unsigned int data_length)
{
...
unsigned short 
RateControlRSSI::calculate_datarate_per(double snr, unsigned int data_length)
{
  vector<double> error_rates(RATE_COUNT); 
...
String
RateControlRSSI::read_handler(Element *e, void *thunk)
{
  RateControlRSSI *src = (RateControlRSSI *)e;
...
int
RateControlRSSI::write_handler(const String &arg, Element *e,
...
CLICK_ENDDECLS
EXPORT_ELEMENT(RateControlRSSI)
