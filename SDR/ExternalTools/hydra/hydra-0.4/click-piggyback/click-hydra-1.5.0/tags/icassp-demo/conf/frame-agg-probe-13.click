...
AddressInfo(me 192.168.101.13 ff:0d:c0:a8:65:0d);
...
// For static routing
AddressInfo(plan12 192.168.101.12 ff:0d:c0:a8:65:0c);
AddressInfo(plan14 192.168.101.14 ff:0d:c0:a8:65:0e);


elementclass MPI {
  // declare
  to_socket::HydraToSocket(UDP, phy_ip, 52131, FRAME 0, SNAPLEN 4096);
  from_socket::HydraFromSocket(UDP, mac_ip, 52132, FRAME 0, SNAPLEN 4096);
...
elementclass FromHydraMAC {
  ...
  rxvector::RXVectorGrNew(VERBOSE 9);

  deagg::DeAggregator(VERBOSE 9);

  // filters
  mpi_filter::PaintSwitch;
  frame_filter::Classifier(0/1c%3f, 0/1d%3f, 0/1b%3f, 0/20%3f, - );
  // extra change needs
  address_filter::Classifier(6/FFFFFFFFFFFF, 6/ff0dc0a8650d, - );
  ...
  unicast_filter [1] -> Print("R CONTROL") -> [1] output ;

  unicast_tee [0] -> decap;
  unicast_tee [1] -> Print("LF") -> lframe -> [1] output; 
...
elementclass ToHydraMAC { $mpi |
  // frame formats
  rts_frame::MACFrameRTS(me:eth, VERBOSE 9);
  ...
  txv::TXVectorGrNew(VERBOSE 9);

  // transmitter
  transmitter::DCFTransmitter(NOTIFIER mpi/gr_phy, SLOTTIME difs, VERBOSE 9);
  ...
  bfilter [0] -> Print("S BROADQ") -> broadcast_queue;
  bfilter [1] -> ub_classifier; //This classifier is used only for tcp ack
  ub_classifier [0] -> Print("S TACK") -> broadcast_queue -> [0] aggregator;
  ub_classifier [1] -> Print("S UNIQ") -> prioritizer -> [1] aggregator;
  
  //input [0] -> Print("C1") -> PullPusher() -> Print("C2") -> ub_classifier;
  //ub_classifier [0] -> Print("C3") -> broadcast_queue -> [0] aggregator;
  //ub_classifier [1] -> Print("C5") -> prioritizer -> [1] aggregator;
  ...
  normal_crc -> Print("NORMAL:BEFORE TXV") -> txv;
  agg_crc -> Print("AGG:BEFORE TXV") ->txv -> Print("S TXVECTOR", NBYTES 40) -> [0] output; 
...
elementclass ToGridDev { 
  ...
  fake_probe::Probe(ETH me:eth, SIZE 100, PERIOD 10000);
  fake_probe->probe_q::Queue(100)-> Print("S PROBE") -> [2] prio;
...
nb::StaticRouteTable(me:eth me:ip, 8,
 plan12:ip plan12:eth plan12:ip 1,
 plan14:ip plan14:eth plan14:ip 1);
