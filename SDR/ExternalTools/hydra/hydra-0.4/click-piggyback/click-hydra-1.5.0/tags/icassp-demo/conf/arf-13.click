...
AddressInfo(me 192.168.155.13 FF:0D:56:BE:19:13);
...
elementclass MPI {
  // declare
  to_socket::HydraToSocket(UDP, phy_ip, 60001, FRAME 0, SNAPLEN 4096);
  from_socket::HydraFromSocket(UDP, mac_ip, 60002, FRAME 0, SNAPLEN 4096);
...
elementclass FromHydraMAC {
  ...
  // extra change needs
  //address_filter::Classifier(4/FFFFFFFFFFFF, 4/FF0D56BE1913, - );
  address_filter::Classifier(6/FFFFFFFFFFFF, 6/FF0D56BE1913, - );
...
elementclass ToHydraMAC { $mpi |
  ...
  // rates
  rc_arf::RateControlArf(24, 10, 2, VERBOSE 9);
  ...
  // tx scheduler
  cp::BEBPolicyArf(rc_arf, MAX_RETRIES 6, MIN_CW 8, VERBOSE 0);
  // for simulation
  ...
  txscheduler::DCFScheduler1(cp, rts_frame, control_rate, rc_arf, set_nav, NOTIFIER mpi/gr_phy, VERBOSE 0);

  // data reception
  encap::HydraEncap(data_frame, VERBOSE 0);
  set_rts::SetRTSThreshold(0, VERBOSE 0);
  //set_rts::SetRTSThreshold(4096, VERBOSE 0);
  set_datarate::SetRateControl(rc_arf, VERBOSE 0);
