...
PHYProcessTime1::PHYProcessTime1()
  : PHYProcessTime(), 
    //_tx_delay(0), _rx_delay(0), _mac_delay(0), _txe_slope(0), 
    //_txe_inter(0), _rxd_slope(0), _rxd_inter(0), _verbose(0)
    _symbol_extension(0.0)
{
...
void*
PHYProcessTime1::cast(const char *n)
{
  if( strcmp(n, "PHYProcessTime1") == 0 )
    return (PHYProcessTime1 *) this;
  else if( strcmp(n, "PHYProcessTime") == 0 )
...
int
PHYProcessTime1::configure(Vector<String> &conf, ErrorHandler *errh)
{
  ...
  if (cp_va_parse(conf, this, errh,
		  ...
		  cpSecondsAsMicro, "Delay in MAC processing", &_mac_delay,
                  cpReal10, "symbole length compared with 4 us", 3, &_symbol_extension,
  		  cpIgnore, 
		  cpIgnore,
		  cpOptional,
		  cpKeywords,
  ...
  cp_spacevec(conf[4], txe_vec);
  cp_spacevec(conf[5], rxd_vec);
...
//Packet * PHYProcessTime1::simple_action(Packet *p)
//{
...
String
PHYProcessTime1::read_param(Element *e, void *thunk)
{
  PHYProcessTime1 *td = (PHYProcessTime1 *)e;
...
int 
PHYProcessTime1::write_param(const String &in_s, Element *e, void *vparam,
		      ErrorHandler *errh)
{
  PHYProcessTime1 *f = (PHYProcessTime1 *)e;
...
*/
CLICK_ENDDECLS
EXPORT_ELEMENT(PHYProcessTime1)
