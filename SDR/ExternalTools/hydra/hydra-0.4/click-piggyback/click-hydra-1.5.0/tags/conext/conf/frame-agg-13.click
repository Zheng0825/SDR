...
AddressInfo(me 192.168.255.13 ff:0d:c0:a8:ff:0d);
AddressInfo(mac_ip 0.0.0.0, phy_ip 0.0.0.0);
//AddressInfo(mac_ip 0.0.0.0, phy_ip 192.168.0.12);
AddressInfo(broadcast 255.255.255.255 FF:FF:FF:FF:FF:FF);

// For static routing
AddressInfo(plan12 192.168.255.12 ff:0d:c0:a8:ff:0c);
AddressInfo(plan14 192.168.255.14 ff:0d:c0:a8:ff:0e);


elementclass MPI {
  // declare
  to_socket::HydraToSocket(UDP, phy_ip, 60001, FRAME 0, SNAPLEN 8192);
  from_socket::HydraFromSocket(UDP, mac_ip, 60000, FRAME 0, SNAPLEN 8192);
...
elementclass FromHydraMAC {
  ...
  /tmp/rate_rx.txt, VERBOSE 9);

  // filters
  mpi_filter::PaintSwitch;
  frame_filter::Classifier(0/1c%3f, 0/1d%3f, 0/1b%3f, 0/20%3f, - );
  // extra change needs
  address_filter::Classifier(6/FFFFFFFFFFFF, 6/ff0dc0a8ff0d, - );
  ...
  mpi_filter[4] -> Print("R AGG", NBYTES 40) -> deagg -> frame_tee[0] -> rx_state;
...
elementclass ToHydraMAC { $mpi |
  ...
  txv::TXVectorGrNew(me:eth, rts_frame, cts_frame, data_frame, ack_frame, sifs, difs, /tmp/rate_tx.txt, VERBOSE 0);
  ...
  //aggregator::UBAggregator(prioritizer, 1, 1);
  ...
  difs::SetDefer(0.100, 0.005, VERBOSE 0);
  phy_time::PHYProcessTime1(0.200, 0.350, 0.005, 10.0, 0.0 0.0, 0.0 0.0, VERBOSE 0);

  pass_csi::PassCSI();

  set_nav::SetDuration1(SIFS sifs, PHYPROCESSTIME phy_time, VERBOSE 9);
  txscheduler::DCFScheduler(cp, rts_frame, control_rate, set_nav, aggregator, NOTIFIER mpi/gr_phy, VERBOSE 9 );
  ...
  size_classifier::Classifier(84/0034, -); //TCP ACK classifier
  ...
  size_classifier [1] -> Print("Q TCP DATA+ACK", NBYTES 200) -> prioritizer-> [1] aggregator;
