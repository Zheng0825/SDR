...
AddressInfo(me 192.168.101.12 ff:0d:c0:a8:65:0c);
...
// For static routing
AddressInfo(plan13 192.168.101.13 ff:0d:c0:a8:65:0d);
AddressInfo(plan14 192.168.101.14 ff:0d:c0:a8:65:0e);


elementclass MPI {
  // declare
  to_socket::HydraToSocket(UDP, phy_ip, 52121, FRAME 0, SNAPLEN 4096);
  from_socket::HydraFromSocket(UDP, mac_ip, 52122, FRAME 0, SNAPLEN 4096);
  gr_phy::GrPhy(VERBOSE 9);

  // composing
  input [0] -> Print("MPI TX") -> [0] gr_phy [0] -> Print("MPI TX2") -> to_socket;
  from_socket -> SetTimestamp -> Print("MPI RX") -> [1] gr_phy [1] -> Print("MPI RX2") -> [0] output;
};
...
elementclass FromHydraMAC {
  ...
  // extra change needs
  address_filter::Classifier(6/FFFFFFFFFFFF, 6/ff0dc0a8650c, - );
...
elementclass ToHydraMAC { $mpi |
  ...
  bfilter [0] -> Print("S BROADQ") -> broadcast_queue -> [0] aggregator;
  ...
  //input [0] -> Print("GO DOWN", NBYTES 200) -> PullPusher() -> Print("TEST0") -> ub_classifier;
  //ub_classifier [0] -> Print("TEST1", NBYTES 200) -> broadcast_queue -> [0] aggregator;
  //ub_classifier [1] -> Print("TEST2", NBYTES 200) -> prioritizer -> [1] aggregator;

  aggregator -> set_rts -> set_datarate ->  Print("GOT AGG DATA", NBYTES 200, PRINTANNO false) -> [0] txscheduler;
