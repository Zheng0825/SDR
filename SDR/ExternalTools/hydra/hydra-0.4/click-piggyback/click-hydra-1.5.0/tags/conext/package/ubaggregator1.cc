...
UBAggregator1::UBAggregator1()
    : _verbose(0), _max_aggsize(0), _max_broadcast(30), _max_unicast(30), _unicast_len(0), _broadcast_len(0), _prioritizer(0)
{
...
int
UBAggregator1::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
int
UBAggregator1::initialize(ErrorHandler *errh)
{
...
String
UBAggregator1::read_param(Element *e, void *thunk)
{
  UBAggregator1 *td = (UBAggregator1 *)e;
...
int 
UBAggregator1::write_param(const String &in_s, Element *e, void *vparam,
		      ErrorHandler *errh)
{
  UBAggregator1 *f = (UBAggregator1 *)e;
...
Packet *
UBAggregator1::make_mpdu_with_pad(Packet *p_in, bool more_data)
{
...
unsigned
UBAggregator1::aggregate(unsigned char **agg_data, u_int32_t *uni_len,
                        u_int32_t *broad_len)
{
  unsigned len = 0;
  unsigned char *ptr;
...
Packet *
UBAggregator1::pull(int port)
{
  int i=0;
  int total_size=0; 

  // wkim: disabling forwards aggregations: no unicast aggregations when
  // broadcast frames exists  
  ...
  while(total_size < _max_aggsize)
  {
     ...
     total_size += _broadcast_buffer[i]->length();
     i++;
  }
  _broadcast_len = i;
  ...
  if (_unicast_len == 0)
  {
    ...
    while(total_size < _max_aggsize)
    {
      ...
      total_size += _unicast_buffer[i]->length();
      i++; 
    }
    _unicast_len = i;
  }

  // Pull packets from the unicase queue: no forward aggregation for experiment
/*  if (_unicast_len == 0)
  {
    for (i = 0; i < 1; i++)
    {
