...
# Title: Batmannoguiofdm
# Generated: Thu Mar 10 19:02:58 2016
##################################################
import threading
...
class batmanNoGuiOFDM(gr.top_block):

    def __init__(self, ampl=0.7, args='', arq_timeout=.1*0 + 0.04, dest_addr=-1, iface='tun0', max_arq_attempts=5 * 2, mtu=128, ogradio_addr=0, port="12345", rate=1e6, rx_antenna="TX/RX", rx_gain=65-20, rx_lo_offset=0, samps_per_sym=4, tx_gain=45, tx_lo_offset=0, tx_freq=915e6, rx_freq=915e6):
        gr.top_block.__init__(self, "Batmannoguiofdm")
    ...
    def set_rx_gain(self, rx_gain):
        with self._lock:
            self.rx_gain = rx_gain
            self.ofdm_radio_0.set_rx_gain(self.rx_gain)
    ...
    def set_tx_gain(self, tx_gain):
        with self._lock:
            self.tx_gain = tx_gain
            self.ofdm_radio_0.set_tx_gain(self.tx_gain)
...
def main(top_block_cls=batmanNoGuiOFDM, options=None):
    if options is None:
        options, _ = argument_parser().parse_args()

    tb = top_block_cls(ampl=options.ampl, args=options.args, arq_timeout=options.arq_timeout, dest_addr=options.dest_addr, iface=options.iface, max_arq_attempts=options.max_arq_attempts, mtu=options.mtu, ogradio_addr=options.ogradio_addr, port=options.port, rate=options.rate, rx_antenna=options.rx_antenna, rx_gain=options.rx_gain, rx_lo_offset=options.rx_lo_offset, samps_per_sym=options.samps_per_sym, tx_gain=options.tx_gain, tx_lo_offset=options.tx_lo_offset, tx_freq=options.tx_freq, rx_freq=options.rx_freq)
    tb.start()
    tb.wait()
